# Versie 0.3.5 - User creationscript
Clear-Host
...
if ($tussenvoegsel) {
    # Voeg een spatie toe na het tussenvoegsel
    $tussenvoegsel += " "
}
$achternaam = Read-Host -Prompt "Voer de achternaam van de gebruiker in"
$achternaam = "$tussenvoegsel$achternaam" # Voeg het tussenvoegsel toe aan de achternaam
...
if ($runSync -eq "" -or $runSync -eq "y" -or $runSync -eq "y") {
    ...
    try {
        $session = New-PSSession -ComputerName "aadconnect"
